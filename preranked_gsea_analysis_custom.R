...
perform_gsea <- function(de_file, 
  ...
  if (pathway_database == "GO") {
    ...
    gsea_results <- gseGO(
      ...
      verbose = TRUE,
      by = "fgsea",
      eps = 0  # Allow P-values smaller than 1e-10
    )
  ...
  if (!is.null(gsea_results) && nrow(gsea_results@result) > 0) {
    ...
    if (nrow(plot_sig_results) > 0) {
      # Dotplot - color by NES, size by -log10(p.adjust)
      tryCatch({
        # First filter significant pathways, then select top 40 by absolute NES
        top_pathways <- plot_sig_results[order(abs(plot_sig_results$NES), decreasing = TRUE), ]
        top_pathways <- head(top_pathways, 40)
        
        # Create a temporary gsea object with just the top pathways
        gsea_top <- gsea_results
        gsea_top@result <- top_pathways
        
        p1 <- dotplot(gsea_top, showCategory = 40, font.size = 10, 
      ...
      if (nrow(pos_pathways) > 0) {
        tryCatch({
          # Sort by NES (highest to lowest) and take top 20 (or fewer)
          pos_pathways <- pos_pathways[order(pos_pathways$NES, decreasing = TRUE), ]
      ...
      if (nrow(neg_pathways) > 0) {
        tryCatch({
          # Sort by NES (lowest/most negative to highest) and take top 20 (or fewer)
          neg_pathways <- neg_pathways[order(neg_pathways$NES), ]
      ...
      tryCatch({
        ...
        # emapplot with explicit NES coloring using aes mapping
        p5 <- emapplot(gsea_results_sim, showCategory = 40, color = "NES") +
          scale_fill_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0, name = "NES") +
...
create_comparative_dotplots <- function(results_list, file_names, output_dir, padj_cutoff = 0.25) {
  ...
  for (i in seq_along(results_list)) {
    if (!is.null(results_list[[i]]) && nrow(results_list[[i]]@result) > 0) {
      df <- results_list[[i]]@result
      # Filter for significance using provided cutoff
  ...
  tryCatch({
    # Get top 20 upregulated from each comparison (by NES)
    top_up_pathways <- c()
    for (comp_name in names(all_data)) {
      df <- all_data[[comp_name]]
      up_df <- df[df$NES > 0, ]
      if (nrow(up_df) > 0) {
        # Sort by NES (highest to lowest) and take top 20
        up_df <- up_df[order(up_df$NES, decreasing = TRUE), ]
        top_up <- head(up_df$ID, 20)
    ...
    if (length(top_up_pathways) > 0) {
      ...
      if (nrow(plot_data_up) > 0) {
        ...
        plot_data_up$Pathway <- sapply(plot_data_up$Pathway, function(x) {
          if (nchar(x) > 60) {
        ...
        p_up <- ggplot(plot_data_up, aes(x = Comparison, y = Pathway)) +
          ...
          labs(title = paste0("Top 20 Upregulated Pathways Across All Comparisons (FDR<", padj_cutoff, ")"),
               x = "Comparison", y = "Pathway", color = "NES")
        
        ggsave(file.path(output_dir, "comparative_dotplot_upregulated_top20.pdf"), 
               p_up, width = max(10, length(all_data) * 1.5), height = max(12, length(unique(plot_data_up$Pathway)) * 0.35))
        ggsave(file.path(output_dir, "comparative_dotplot_upregulated_top20.png"), 
               p_up, width = max(10, length(all_data) * 1.5), height = max(12, length(unique(plot_data_up$Pathway)) * 0.35), dpi = 300)
        message(paste("Comparative upregulated dotplot saved (", length(unique(plot_data_up$Pathway)), "unique pathways)"))
      }
    }
  }, error = function(e) {
  ...
  tryCatch({
    # Get top 20 downregulated from each comparison (by NES, most negative)
    top_down_pathways <- c()
    for (comp_name in names(all_data)) {
      df <- all_data[[comp_name]]
      down_df <- df[df$NES < 0, ]
      if (nrow(down_df) > 0) {
        # Sort by NES (lowest/most negative to highest) and take top 20
        down_df <- down_df[order(down_df$NES), ]
        top_down <- head(down_df$ID, 20)
    ...
    if (length(top_down_pathways) > 0) {
      ...
      if (nrow(plot_data_down) > 0) {
        # Wrap long pathway names
        plot_data_down$Pathway <- sapply(plot_data_down$Pathway, function(x) {
          if (nchar(x) > 60) {
            paste(strwrap(x, width = 60), collapse = "\n")
          } else {
            x
        ...
        p_down <- ggplot(plot_data_down, aes(x = Comparison, y = Pathway)) +
          ...
          theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 10),
                ...
                legend.position = "right",
                plot.margin = margin(5, 5, 5, 5, "mm")) +
          labs(title = paste0("Top 20 Downregulated Pathways Across All Comparisons (FDR<", padj_cutoff, ")"),
               x = "Comparison", y = "Pathway", color = "NES")
        
        ggsave(file.path(output_dir, "comparative_dotplot_downregulated_top20.pdf"), 
               p_down, width = max(10, length(all_data) * 1.5), height = max(12, length(unique(plot_data_down$Pathway)) * 0.35))
        ggsave(file.path(output_dir, "comparative_dotplot_downregulated_top20.png"), 
               p_down, width = max(10, length(all_data) * 1.5), height = max(12, length(unique(plot_data_down$Pathway)) * 0.35), dpi = 300)
        message(paste("Comparative downregulated dotplot saved (", length(unique(plot_data_down$Pathway)), "unique pathways)"))
      }
    }
  }, error = function(e) {
...
generate_html_report <- function(results_list, file_names, output_dir, padj_cutoff, pathway_database) {
...
'))
    ...
    comp_up_base64 <- image_to_base64(file.path(output_dir, "comparative_dotplot_upregulated_top20.png"))
    comp_down_base64 <- image_to_base64(file.path(output_dir, "comparative_dotplot_downregulated_top20.png"))
...
'))
    ...
    # Section 5: Tree Plots with tabs
    html_content <- tagList(html_content, HTML('
    <div class="section">
...
'))
    ...
    # Section 6: Enrichment Maps with tabs
    html_content <- tagList(html_content, HTML('
    <div class="section">
...
")
  ...
  create_comparative_dotplots(all_gsea_results, tsv_files, output_dir, padj_cutoff)
  
  # Create upset plot for pathway overlap
  create_upset_plot(all_gsea_results, tsv_files, output_dir, padj_cutoff)
  
  # Generate interactive HTML report
  generate_html_report(all_gsea_results, tsv_files, output_dir, padj_cutoff, pathway_database)
